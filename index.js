...
export default class VideoPlayer extends Component {
  ...
  _togglePlayPause() {
    ...
    if(state.isReachEnd) {
      this.seekTo(0)
      return this.setState({ isReachEnd: false, paused: false }, () => this.hideControlAnimation())
    }
  ...
  constrainToVolumeMinMax(val = 0) {
    ...
    if (val >= this.player.volumeWidth + 9) {
  ...
  renderPlayControl(children, callback, style = {}) {
    return (
      <TouchableHighlight
        underlayColor="transparent"
        activeOpacity={0.3}
  ...
  renderTopControls() {
    const backControl = this.props.disableBack
      ...
      : this.renderBack()
    const volumeControl = this.renderNullControl()
  ...
  renderBottomControls() {
    ...
    const seekbarControl = this.props.disableSeekbar
      ? this.renderNullControl()
      : this.renderSeekbar()
    const currentTimer = this.props.disableTimer
      ? this.renderNullControl()
      : this.renderCurrentTimer()
   ...
   */
  renderPlayPause() {
    const { paused, isReachEnd } = this.state
    const name = paused ? 'playcircleo' : 'pausecircleo'
    return this.renderPlayControl(
      <Animated.View
        style={{
          alignItems: 'center',
      ...
      >
        {
          !isReachEnd?
          <AntDesign size={50} name={name} color="white" /> :
          <MaterialCommunityIcons size={50} name="replay" color="white" />
        }
      </Animated.View>,
      this.methods.togglePlayPause,
    )
